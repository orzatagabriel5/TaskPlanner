package com.taskPlanner.weekView;
...
public class WeekView extends View {
    ...
    private int minVisibleHour = 8;
    private int maxVisibleHour = 22;
            ...
            switch (mCurrentFlingDirection) {
                ...
                case VERTICAL:
                    mScroller.fling(
                            ...
                            (int) mCurrentOrigin.y, 0,
                            (int) velocityY,
                            Integer.MIN_VALUE,
                            Integer.MAX_VALUE,
                            (int) -(mHourHeight * (maxVisibleHour + 0.1) + mHeaderTextHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom + mTimeTextHeight/2 - getHeight()),
                            (int) -(mHourHeight * (minVisibleHour - 0.1)));
                    break;
            }
    ...
    private void drawHeaderRowAndEvents(Canvas canvas) {
        ...
        if (mCurrentOrigin.y < -(mHourHeight * (maxVisibleHour + 0.1) + mHeaderTextHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom + mTimeTextHeight/2 - getHeight()))
            mCurrentOrigin.y = (float) -(mHourHeight * (maxVisibleHour + 0.1) + mHeaderTextHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom + mTimeTextHeight/2 - getHeight());

        if (mCurrentOrigin.y > -mHourHeight * minVisibleHour)
            mCurrentOrigin.y = -mHourHeight * minVisibleHour;
            ...
            for (int i = 0; i < mEventRects.size(); i++) {
                if (isSameDay(mEventRects.get(i).event.getStartTime(), date)) {
                    ...
                    if (left < right &&
                        ...
                        //Color event with services color and duration of each
                        float nextTop = top;
                        float bottomMtop = bottom - top;
                        for(Service service : mEventRects.get(i).event.getServiceList()){
                            float valueFromServices = (float) service.getDuration() / (float) mEventRects.get(i).event.getTotalDuration();
                            float nextBottom = nextTop + bottomMtop * valueFromServices;
                            mEventRects.get(i).rectF.set(left, nextTop, right, nextBottom);
                            mEventBackgroundPaint.setColor(ContextCompat.getColor(getContext(),service.getColorId()));

                            nextTop = nextBottom;
                            canvas.drawRoundRect(mEventRects.get(i).rectF, mEventCornerRadius, mEventCornerRadius, mEventBackgroundPaint);
                        }
                        if(mEventRects.get(i).event.getServiceList().size() < 1){
                            canvas.drawRoundRect(mEventRects.get(i).rectF, mEventCornerRadius, mEventCornerRadius, mEventBackgroundPaint);
                        }
                        //make all event selectable
...
//         Prepare the location of the event.
    ...
    private void cacheEvent(WeekViewEvent event) {
        ...
        if (!isSameDay(event.getStartTime(), event.getEndTime())) {
            ...
            WeekViewEvent event1 = new WeekViewEvent(event.getId(), event.getName(), event.getLocation(), event.getStartTime(), endTime, event.getTotalDuration(), event.getServiceList());
            ...
            otherDay.add(Calendar.DATE, 1);
                ...
                WeekViewEvent eventMore = new WeekViewEvent(event.getId(), event.getName(), overDay, endOfOverDay, event.getTotalDuration(), event.getServiceList());
            ...
            WeekViewEvent event2 = new WeekViewEvent(event.getId(), event.getName(), event.getLocation(), startTime, event.getEndTime(), event.getTotalDuration(), event.getServiceList());
    ...
    /**
     * @return The number of min visible hour
     */
    public int getMinVisibleHour() {
        return minVisibleHour;
    }

    /**
     * @param minVisibleHour Set the min visible hour
     */
    public void setMinVisibleHour(int minVisibleHour) {
        if(minVisibleHour >= 0 && minVisibleHour < 12) {
            this.minVisibleHour = minVisibleHour;
        }else{
            this.minVisibleHour = 0;
        }
    }

    /**
     * @return The number of max visible hour
     */
    public int getMaxVisibleHour() {
        return maxVisibleHour;
    }

    /**
     * @param maxVisibleHour The number of max visible hour
     */
    public void setMaxVisibleHour(int maxVisibleHour) {
        if(maxVisibleHour >= 8 && maxVisibleHour < 24) {
            this.maxVisibleHour = maxVisibleHour;
        }else{
            this.maxVisibleHour = 24;
        }
    }
